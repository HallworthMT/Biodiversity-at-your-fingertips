...
existing_users=$(grep ^biodiv_ /etc/passwd | sed 's/:.*//')
...
for id in $(seq 1 $1); do
    ...
    adduser \
        ...
        --disabled-password \
        biodiv_$id < /dev/null
    cp -a /home/mhallworth/ $userhome
    chown -R biodiv_$id $userhome
    echo "biodiv_$id:NENHC2023" | chpasswd
done
